...
test.describe("Insights section of the form", () => {
  ...
  test("Should show a section with the current saved insights plus an add new insight card", () => {
  ...
  test("Should show an empty section if no previous insight is present, only new insight card is present", () => {
    cy.intercept(
      "GET",
      `${Cypress.env("REACT_APP_API_URL")}/campaigns/4904/ux`,
      {
  ...
  test("Should show an empty form to create a new insight when clicking on the add new insight card", () => {
  ...
  test("Should show an error for each field if trying to save an empty insight", () => {
    cy.dataQa("add-new-insight").click({ force: true });
  ...
  test("Should show a prefilled form when clicking on the edit insight", () => {
